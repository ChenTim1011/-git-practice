...
假設還沒有 proxy ，分成用戶端和伺服器端，用戶端發送請求到伺服器端，伺服器接收訊息回傳給用戶端，中間沒有人。 proxy 就像是中間代理人的角色。
...
- [x] 解釋 `Reverse proxy` vs `Forward Proxy`
  ...
  Forward Proxy: 代理用戶端的請求，可幫助用戶端存取伺服器，可以繞過防火牆或網路限制、隱藏用戶端的 IP。

  Reverse proxy: 代理伺服器的請求，可幫助伺服器處理和分發用戶端的請求，常用於分配流量、隱藏內部伺服器。
