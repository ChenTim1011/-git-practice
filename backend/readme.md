...
1. 嘗試連線到特定網站或伺服器：
您可以使用 cURL 命令嘗試連線到一個已知的網站或伺服器，例如 google.com。如果連線成功，cURL 會回傳該網站的 HTML 內容，否則會回傳錯誤訊息。
```bash
curl www.google.com 
```
3. 檢查 HTTP 狀態碼：
使用 -I 選項請求伺服器回傳 HTTP 標頭，並檢查狀態碼是否為 200 OK，表示連線成功。
```bash
...
5. -I ： 只請求伺服器回傳 HTTP 標頭，而不下載檔案內容。可以用於檢查伺服器的狀態、檔案大小等等。
